...
echo.
echo ===========================================
echo    GitHub 超级工具箱 v3.1
echo    融合11个脚本功能，已修复所有已知bug
echo ===========================================
echo.
echo  1. 克隆仓库 + 智能配置环境
...
echo ===========================================
echo    【同步云端到本地】
echo    功能说明：将远程仓库最新代码拉取到本地
echo    适用场景：本地代码落后 / 团队协作更新
echo    注意：若本地有未提交修改，建议先 stash
echo ===========================================
echo.
...
if %HAS_LOCAL_CHANGES% neq 0 (
    echo [WARN] 检测到本地有未提交的修改！
    echo.
    choice /C YN /N /M "是否先 stash 保存本地修改？(Y=保存 / N=直接丢弃): "
    if %errorlevel%==1 (
    ...
    echo [WARN] 检测到暂存区有未提交的修改！
    choice /C YN /N /M "是否先 stash 保存？(Y=保存 / N=直接丢弃): "
    if %errorlevel%==1 (
        git stash push -m "工具箱自动保存: %DATE% %TIME%"
        set "DID_STASH=1"
    ) else (
...
if %errorlevel% neq 0 (
    ...
    echo.
    echo 常见原因及解决办法：
    echo   1. SSH 22 端口被拦截（校园网/公司网常见）
    echo      解决：改用 HTTPS 协议，或配置 SSH 走 443 端口
    echo      操作：选择菜单【4】修改远程地址为 HTTPS
    echo   2. 网络连接问题
    echo      解决：检查网络或代理设置
    echo   3. 远程仓库不存在或没有权限
    echo      解决：确认仓库地址和访问权限
...
if %PULL_RESULT% equ 0 (
    ...
    echo [SUCCESS] 同步完成！
    echo ===========================================
    echo 本地仓库已更新为 %REMOTE_NAME%/%REMOTE_BRANCH% 最新状态
    echo.
    for /f "tokens=*" %%c in ('git log -1 --oneline') do echo 最新提交: %%c
    echo.
    if "%DID_STASH%"=="1" (
        echo [INFO] 之前 stash 了本地修改，是否恢复？
        choice /C YN /N /M "恢复本地修改？(Y=恢复 / N=保留在stash中): "
        if %errorlevel%==1 (
            git stash pop
            echo [SUCCESS] 本地修改已恢复
        ) else (
            echo [INFO] 修改保留在 stash 中，稍后手动恢复: git stash pop
        )
    )
) else (
    echo.
    ...
    echo.
    echo 解决冲突步骤：
    echo   1. 打开冲突文件，找到 ^<^<^<^<^<^<^< HEAD 标记
    echo   2. 手动选择保留的代码
    echo   3. 执行: git add .
    echo   4. 执行: git commit -m "解决冲突"
    echo.
    if "%DID_STASH%"=="1" (
        echo [INFO] 之前 stash 的修改仍保留在 stash 中
        echo [INFO] 冲突解决后可恢复: git stash pop
    )
)
